...
export interface WebhooksSectionActions {
  ...
  createWebhook: (channelId: string, displayName: string, avatarUrl: string | null) => Promise<WebhookCreatedResult>;
  regenerateWebhook: (id: string) => Promise<WebhookCreatedResult>;
  deleteWebhook: (id: string) => Promise<void>;
}

interface Props {
  channels: Channel[];
  actions: WebhooksSectionActions;
}
...
export function WebhooksSection({ channels, actions }: Props) {
  ...
  async function loadWebhooks() {
    try {
      const list = await actions.loadWebhooks();
      setWebhooks(list);
    } catch (e) {
      setError(String(e));
    }
  ...
  async function handleCreate() {
    ...
    try {
      const result = await actions.createWebhook(channelId, displayName.trim(), avatarUrl.trim() || null);
  ...
  async function handleRegenerate(id: string) {
    ...
    try {
      const result = await actions.regenerateWebhook(id);
