...
export function IdentityBackupSection({ publicKey, onImported }: Props) {
  ...
  async function handleExport() {
    ...
    setError(null);
    try {
      const record = await loadIdentity();
      if (!record) throw new Error("No identity found.");

      const identityJson = JSON.stringify({ seed_hex: record.seed_hex, security_nonce: record.security_nonce, security_level: record.security_level });
      const enc = new TextEncoder();
      const saltArr = crypto.getRandomValues(new Uint8Array(16));
      const nonceArr = crypto.getRandomValues(new Uint8Array(12));

      const keyMaterial = await crypto.subtle.importKey(
        "raw", enc.encode(exportPassphrase).buffer as ArrayBuffer, "PBKDF2", false, ["deriveKey"],
      );
  ...
  async function handleImport() {
    ...
    try {
      ...
      const keyMaterial = await crypto.subtle.importKey(
        "raw", enc.encode(importPassphrase).buffer as ArrayBuffer, "PBKDF2", false, ["deriveKey"],
      );
      const aesKey = await crypto.subtle.deriveKey(
      ...
      try {
        plaintext = await crypto.subtle.decrypt(
          { name: "AES-GCM", iv: nonceBuf.buffer as ArrayBuffer },
          aesKey,
      ...
      const parsed = JSON.parse(identityJson) as { seed_hex?: string; security_nonce?: number; security_level?: number };
      if (!parsed.seed_hex) throw new Error("Invalid backup content.");

      const existing = await loadIdentity();
      if (existing) {
        if (existing.seed_hex === parsed.seed_hex) {
          setError("This backup is the identity already on this device. Nothing to do.");
          return;
        }
        if (!confirm(`This device already has a different Voxply identity (${existing.seed_hex.slice(0, 8)}…). Replace it?\n\nMake sure that identity is backed up first.`)) {
          return;
        }
      }
      ...
      const newRecord: IdentityRecord = {
        ...
        security_nonce: parsed.security_nonce ?? 0,
        security_level: parsed.security_level ?? 0,
      };
      await saveIdentity(newRecord);
    ...
    <div className="settings-section">
      ...
      {step === "export-form" && (
        <div>
          ...
          <input
            type="text"
            placeholder="Label (optional, e.g. tablet backup June 2026)"
