...
interface Props {
   ...
   *  below it are assignable here, matching the member-list context menu
   *  and the hub's own guard. */
  myMaxPriority: number;
  onChanged: (roles: RoleInfo[]) => void;
}

// Chip list + a "manage roles" popover, for inline role editing in the
// hub-admin Members table. Mirrors UserContextMenu's role-toggle behavior.
export function MemberRoleManager({ pubkey, currentRoles, myMaxPriority, onChanged }: Props) {
  ...
  async function toggle(role: RoleInfo) {
    ...
    setError(null);
    try {
      if (has) await removeRoleFromUser(pubkey, role.id);
      else await assignRoleToUser(pubkey, role.id);
