...
import type { HubBadge, PendingBadgeOffer } from "@shared/types";
import {
  getDiscoveryTags,
  setDiscoveryTags,
  listBadges,
...
export function ServerTagsSection({ hubUrl: _hubUrl }: Props) {
  ...
  async function loadTags() {
    try {
      const s = await getDiscoveryTags();
  ...
  async function loadBadgeData() {
    ...
    try {
      const [b, p] = await Promise.all([listBadges(), listPendingBadges()]);
      setBadges(b);
      setPendingBadges(p);
  ...
  async function handleSaveTags() {
    ...
    try {
      ...
      await setDiscoveryTags(tags, nsfw);
      setSaveStatus("saved");
  ...
  async function handleAccept(id: string) {
    try { await acceptBadge(id); await loadBadgeData(); } catch { /* ignore */ }
  }

  async function handleDecline(id: string) {
    try { await declineBadge(id); await loadBadgeData(); } catch { /* ignore */ }
  }

  async function handleRemoveBadge(id: string) {
    try { await removeBadge(id); await loadBadgeData(); } catch { /* ignore */ }
  }
  ...
  async function handleGrantBadge() {
    ...
    try {
      await grantBadge(grantTargetUrl.trim(), grantLabel.trim());
      setGrantStatus("ok");
      setGrantTargetUrl("");
      setGrantLabel("");
